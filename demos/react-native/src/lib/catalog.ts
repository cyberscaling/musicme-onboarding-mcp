// demos/react-native/src/lib/catalog.ts
/**
 * Catalog API client. Direct port of demos/webapp/public/catalog.ts with
 * RN-specific URL construction (no `location.origin`). Hits the same
 * /api/catalog/* routes on demo-worker.
 */
...
export function adaptAlbumAliased(
  r: AliasedAlbum & { cb?: number | string; album?: string; street_date?: string; style?: number },
): Album {
...
function buildUrl(path: string, query?: Record<string, string | number | undefined>): string {
  ...
  if (query) {
    for (const [k, v] of Object.entries(query)) {
      if (v != null && v !== '') params.push(`${encodeURIComponent(k)}=${encodeURIComponent(String(v))}`)
    }
  }
  const qs = params.length > 0 ? `?${params.join('&')}` : ''
  return `${WEBAPP_URL}${path}${qs}`
}

async function get<T>(path: string, query?: Record<string, string | number | undefined>): Promise<T> {
  const r = await fetch(buildUrl(path, query), { credentials: 'include' })
...
export const catalog = {
  ...
  album: async (cb: string): Promise<{ album: Album; tracks: Track[] }> => {
    ...
    const tracks = tracksJson.map(adaptTrackRaw).sort((a, b) => a.disc - b.disc || a.track - b.track)
    return { album, tracks }
  },
